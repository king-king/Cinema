library( function () {
    ...
    function materialize( page, data, callback ) {
        ...
        object.foreach( data, function ( id, plugin ) {
            ...
            if ( plugin.name == "page" ) {
                ...
                // 所有的滤镜全都统一成异步回掉的形式，具体就是在getPageData上体现。
                // 关于异步处理问题，这里的处理有一个基本的原则就是，首先不管有没有滤镜都会生成一个含有page内容的canvas，
                // 不会等待是否完成滤镜才会播放，而是播放，如果滤镜数据出来了，就加上去
