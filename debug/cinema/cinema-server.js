...
MongoClient.connect( dburl, function ( err, db ) {
    if ( err ) {
        ...
        http.createServer( function ( req, res ) {
            ...
            // 插入一条数据
            else if ( req.url == "/insert-cinema-data" ) {
                doInsertCinema();
            }
            // 检查动画名字是否被占用
            ...
            // 得到动画数据
            else if ( req.url == "/get-cinema-list" ) {
                doGetCinemaList();
            }
                ...
                (new multiparty.Form()).parse( req, function ( err, fields, files ) {
                        ...
                        fs.readFile( files["wq-file"][0].path, null, function ( err, buffer ) {
                            if ( err ) {
                                ...
                                checkExist( db, hex, function ( err, isExist, alias ) {
                                    if ( err ) {
                                        res.end( JSON.stringify( {
                                            code : 604,
                                            message : "校验失败"
                                        ...
                                        res.end( JSON.stringify( {
                                            code : 602,
                                            message : "此文件已经上传过，和" + alias + "是一样的"
                                        } ) );
                                    }
                                    else {
                                        fs.writeFile( "img/plugin/" + files["wq-file"][0].originalFilename, buffer, function ( err ) {
                                            if ( !err ) {
                                                // 将记录写入到数据库中
            ...
            function doInsertCinema() {
                ...
                req.on( "end", function () {
                    ...
                    postdata = postdata.replace( "------WebKitFormBoundary", "" );
                    var name = postdata.substring( postdata.indexOf( "name=\"name\"" ) + 11, postdata.indexOf( "------WebKitFormBoundary" ) ).trim();
                    insertOneCinema( db, name, data, function ( err ) {
                        if ( err ) {
                            res.end( JSON.stringify( {
                                code : 1201,
                                message : "数据库故障"
            ...
            function doGetCinemaList() {
                getCinemaList( db, function ( err, data ) {
                    res.writeHead( 200, {'Content-Type' : 'text/plain', "Access-Control-Allow-Origin" : "*"} );
                    if ( err ) {
                        res.end( JSON.stringify( {
                            code : 1100,
        ...
        } ).listen( 7474, os.networkInterfaces().WLAN[1].address );
    }
} );
...
function insertIntoPluginCollection( db, data, callback ) {
    ...
    customPluginCollection.insertOne( {name : data.name, hash : data.hash, alias : data.alias}, callback );
}
...
function checkExist( db, hash, callback ) {
    ...
    customPluginCollection.find( {hash : hash} ).toArray( function ( err, docs ) {
        callback( err, docs.length != 0, docs[0] ? docs[0].alias : null );
    } );
}
...
function insertOneCinema( db, name, animate, callback ) {
    !cinemaCollection && (cinemaCollection = db.collection( "cinemadata" ));
    cinemaCollection.insertOne( {
        name : name,
        animate : animate
    }, callback );
}
...
function getCinemaList( db, callback ) {
    !cinemaCollection && (cinemaCollection = db.collection( "cinemadata" ));

    cinemaCollection.find( {} ).toArray( callback );
}